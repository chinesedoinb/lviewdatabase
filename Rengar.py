...
def Combo(game):
    ...
    if game.player.mana <= 3.0 and not getBuff(game.player,"RengarR"):
        ...
        if use_e_in_combo and IsReady(game,e_spell):
            ...
            if target:
                            ...
                            e_travel_time = disToPlayer/1500
...
def LaneClear(game):
    ...
    if use_w_in_lane and IsReady(game,w_spell):
            target=GetBestMinionsInRange(game,350)
            if target:
                if game.player.W.timeCharge>0:
                    w_spell.trigger(False)
        
    if use_e_in_lane and IsReady(game,e_spell):
    ...
    if use_w_in_lane and IsReady(game,w_spell):
            target=GetBestJungleInRange(game,350)
            if target:
                if game.player.W.timeCharge>0:
                    w_spell.trigger(False)
