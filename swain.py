...
WRange=[5500 , 6000 , 6500 , 7000 , 7500]
def Combo(game):
    global use_q_in_combo, use_w_in_combo, use_e_in_combo, use_r_in_combo,spellQTimer
    ...
    if use_w_in_combo and IsReady(game, w_spell) :
                target=TargetSelector(game,WRange[game.player.W.level -1])
                if target and game.is_point_on_screen(target.pos) :
                    q_travel_time = (2000/1000 ) *0.50
                    ...
                    if game.player.pos.distance (predicted_target.pos) <= WRange[game.player.W.level -1]:
    ...
    if use_e_in_combo and IsReady(game, e_spell) :
                ...
                if targetQ and game.is_point_on_screen(targetQ.pos):       
                    if game.player.pos.distance (targetQ.pos) >= 400 :
                            ...
                            e_travel_time = 850/935
                            predicted_pos = predict_pos (targetQ, e_travel_time)
                            predicted_target = Fake_target (targetQ.name, predicted_pos, targetQ.gameplay_radius)
                            if game.player.pos.distance (targetQ.pos) <= 850 :
                    ...
                    elif game.player.pos.distance (targetQ.pos) <= 400 :
                            
                            disToPlayer=game.player.pos.distance (targetQ.pos)
                            e_travel_time = disToPlayer/935
                            predicted_pos = predict_pos (targetQ, e_travel_time)
                            predicted_target = Fake_target (targetQ.name, predicted_pos, targetQ.gameplay_radius)
                            if  game.player.mana >= skillMana(game,e_spell,e_spell.level):
                                    e_spell.move_and_trigger(game.world_to_screen(predicted_target.pos))

                targetQ2=TargetSelector (game,2000)
                if targetQ2 :
                    if e_spell.name == "swaine2":
                        e_spell.trigger(False)              

    if use_r_in_combo and IsReady(game, r_spell) :
        
                targetR=TargetSelector(game,675)
                hp_player= int(game.player.health / game.player.max_health * 100)
                if targetR:
                    if not r_spell.name == "swainrsoulflare" and hp_player<=70:
                        r_spell.trigger(False)

                    if  r_spell.name == "swainrsoulflare":
                            hp = int(targetR.health / targetR.max_health * 100)
                            if game.player.pos.distance (targetR.pos) <= 675 :
