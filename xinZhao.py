...
def Combo(game):
    ...
    if use_q_in_combo and IsReady(game, q_spell) :
                target = TargetSelector (game,350)
                if target :
    ...
    if use_r_in_combo and IsReady(game, r_spell) :
                ...
                if targetR:
                            TargetHP = int(targetR.health / targetR.max_health * 100)
                            PlayerHP = int(game.player.health / game.player.max_health * 100)
                            if game.player.pos.distance (targetR.pos) <= 750 :
                                if  game.player.mana >= 100:
                                    if RDamage(game, targetR) >= targetR.health or TargetHP<=30:
                                        r_spell.trigger(False) 
                            if getCountR(game,750) and PlayerHP<80:
                                r_spell.trigger(False)
...
def Laneclear(game):
    ...
    if lane_clear_with_q and IsReady(game, q_spell) :
                targetQ = GetBestMinionsInRange(game,350)
                if targetQ :
                    if  game.player.mana >= 30:
...
def Jungleclear(game):
    ...
    before_cpos = game.get_cursor()
    if lane_clear_with_q and IsReady(game, q_spell) :
                targetQ = GetBestJungleInRange(game,350)
                if targetQ :
                    if  game.player.mana >= 30:
                        q_spell.move_and_trigger(game.world_to_screen(targetQ.pos))
