...
winstealer_script_info = {
    "script": "SA1-Miss fortune",
    "author": "SA1",
    "description": "SA1-Missfortune",
    "target_champ": "missfortune",
}
...
def winstealer_draw_settings(game, ui):
    ...
    ui.text("SA1-Miss fortune : 1.0.0.0")
    ui.separator ()
...
def Combo(game):
    ...
    if use_q_in_combo and IsReady(game, q_spell) :
                ...
                if target :
                    if game.player.mana >= Q_mana[game.player.Q.level -1]:
...
def Laneclear(game):
    ...
    if lane_clear_with_w and IsReady(game, w_spell) :
                target=GetBestMinionsInRange(game,game.player.atkRange + game.player.gameplay_radius+25)
                if target:
                    if  game.player.mana >= 45:
                        w_spell.trigger(False)
...
def Jungleclear(game):
    ...
    if jungle_clear_with_w and IsReady(game, w_spell) :
                target=GetBestJungleInRange(game,game.player.atkRange + game.player.gameplay_radius+25)
                if target:
                    if  game.player.mana >= 45:
                        w_spell.trigger(False)
