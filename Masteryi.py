...
winstealer_script_info = {
    "script": "SA1-Master Yi",
    "author": "SA1",
    "description": "SA1-Master Yi",
    "target_champ": "masteryi",
}
...
def winstealer_load_cfg(cfg):
    ...
    global jungle_clear_with_q, jungle_clear_with_w, jungle_clear_with_e,smart_combo,use_q_in_evade
    ...
    use_q_in_evade=cfg.get_bool("use_q_in_evade",True)
...
def winstealer_save_cfg(cfg):
    ...
    cfg.set_bool("use_q_in_evade", use_q_in_evade)
...
def winstealer_draw_settings(game, ui):
    ...
    if ui.treenode("Combo Settings"):
        ...
        ui.separator ()
        use_q_in_evade = ui.checkbox("Use Q for Evade", use_q_in_evade)
        ui.separator ()
        ui.separator ()
        ui.text("<< R manual >>")
        ui.separator ()
        ...
        if InSkillShot( game, game.player.pos, missile, spell, game.player.gameplay_radius * 2) and game.is_point_on_screen(missile.pos):
            ...
            if use_q_in_evade:    
                if IsReady(game, q_spell) and game.player.mana>=50:
                    ...
                    targetMinion=GetBestTargetsInRange (game,700)
                    if targetMinion:
                        if game.player.mana >= 60:
                                    q_spell.move_and_trigger(game.world_to_screen(targetMinion.pos))
...
def Combo(game):
    ...
    if use_e_in_combo and IsReady(game, e_spell) :
                target=TargetSelector(game,370)
                if target:
