...
winstealer_script_info = {
    "script": "SA1-Morgana",
    "author": "SA1",
    "description": "SA1-Morgana",
    "target_champ": "morgana",
}
...
def winstealer_draw_settings(game, ui):
    ...
    if ui.treenode("Jungle Clear Settings"):
        ...
        jungle_clear_with_w = ui.checkbox("Jungle with E", jungle_clear_with_w)
        ui.treepop()
...
def AntiGap(game):
    ...
    if IsReady(game, q_spell) and use_q_in_combo:
                if target :
                    
                    if game.player.pos.distance (target.pos) <= 370 and game.player.mana >= 50 and not IsCollisioned(game, target):
                                    game.move_cursor(game.world_to_screen(target.pos))
                                    time.sleep(0.01)
                                    q_spell.trigger(False)
                                    time.sleep(0.01)
                                    game.move_cursor(before_cpos)
    if IsReady(game, e_spell) and use_e_in_combo:
              if target :
                    if game.player.pos.distance (target.pos) <= 370 and game.player.mana >= 80 and lastE + 2 < game.time:
...
def Evade(game):
    ...
    for missile in game.missiles:
        ...
        if InSkillShot(
            ...
            if use_e_in_combo:
                if IsReady(game, e_spell) and game.player.mana>=80 and lastE + 2 < game.time:
                    e_spell.move_and_trigger(game.world_to_screen(game.player.pos))
                    lastE=game.time
lastq=0
def Combo(game):
    ...
    if use_q_in_combo and IsReady(game, q_spell) :
                targetQ = TargetSelector (game,1300)
                if targetQ :
                            ...
                            if  game.player.mana >= 70 and lastq+1<game.time and not is_collisioned(game,predicted_target):
    ...
    if use_w_in_combo and IsReady(game, w_spell) :
            ...
            targetQ = game.GetBestTarget(
                UnitTag.Unit_Champion,
                900
            )
            if targetQ:
                w_travel_time = 900/20
                predicted_pos = predict_pos (targetQ, w_travel_time)
                predicted_target = Fake_target (targetQ.name, predicted_pos, targetQ.gameplay_radius)
...
def Laneclear(game):
    ...
    if lane_clear_with_q and IsReady(game, q_spell) :
            ...
            targetQ = GetBestMinionsInRange(game,1300)
            if targetQ:
                                if game.player.pos.distance (targetQ.pos) <= 1300 :
                                    if  game.player.mana >= 70:
    ...
    if lane_clear_with_w and IsReady(game, w_spell) :
            ...
            if targetQ:
                                if game.player.pos.distance (targetQ.pos) <= 900 :
                                    if  game.player.mana >= 70:
                                        game.move_cursor(game.world_to_screen(targetQ.pos))
                                        w_spell.trigger(False)
                                        time.sleep(0.01)
...
def Jungleclear(game):
    ...
    if lane_clear_with_q and IsReady(game, q_spell) :
            ...
            targetQ = GetBestJungleInRange(game,1300)
            if targetQ:
                                if game.player.pos.distance (targetQ.pos) <= 1300 :
                                    if  game.player.mana >= 70:
                                        game.move_cursor(game.world_to_screen(targetQ.pos))
