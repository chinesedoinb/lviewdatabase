...
winstealer_script_info = {
    "script": "SA1-blitzcrank",
    "author": "SA1",
    "description": "SA1-blitzcrank",
...
def winstealer_draw_settings(game, ui):
    ...
    ui.text("SA1-BlitzCrank: 1.0.0.0")
    ui.separator ()
...
spellTimer = Timer()
def Combo(game):
    global use_q_in_combo, use_w_in_combo, use_e_in_combo, use_r_in_combo,spellTimer
    ...
    before_cpos = game.get_cursor()
    if use_q_in_combo and IsReady(game, q_spell) and game.player.mana>=100:
                targetQ = TargetSelector (game,1115)
                if targetQ :
                    if game.player.pos.distance (targetQ.pos) >= 600 :
                            
                            disToPlayer=game.player.pos.distance (targetQ.pos)
                            e_travel_time = 1115/1800
                            ...
                            if game.player.pos.distance (targetQ.pos) <= 1000 :
                                if spellTimer.Timer() and not is_collisioned(game,predicted_target):
                                        q_spell.move_and_trigger(game.world_to_screen(predicted_target.pos))
                                        spellTimer.SetTimer(0.4)   
                    elif game.player.pos.distance (targetQ.pos) <= 600 :
                            
                            disToPlayer=game.player.pos.distance (targetQ.pos)
                            e_travel_time = disToPlayer/1800
                            predicted_pos = predict_pos (targetQ, e_travel_time)
                            predicted_target = Fake_target (targetQ.name, predicted_pos, targetQ.gameplay_radius)
                            
                            if spellTimer.Timer() and not is_collisioned(game,targetQ):
                                    q_spell.move_and_trigger(game.world_to_screen(predicted_target.pos))
                                    spellTimer.SetTimer(0.4)           

    if use_e_in_combo and IsReady(game, e_spell) and game.player.mana>=25:
                
                targetR=TargetSelector(game,490)
                if targetR:
                    if game.player.pos.distance (targetR.pos) <= 300 :
    ...
    if use_r_in_combo and IsReady(game, r_spell) and game.player.mana>=100:
        ...
        if targetR:
            ...
            if getBuff(targetR,"rocketgrab2") or getBuff(targetR,"stun"):
                r_spell.trigger(False)
            if getBuff(targetR,"powerfistslow") :
                r_spell.trigger(False)
