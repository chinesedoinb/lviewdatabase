...
manaE=[30,35,40,45,50]      
manaW=[80,70,60,50,40]     
def Combo(game):
    global use_q_in_combo, use_w_in_combo, use_e_in_combo, use_r_in_combo
    ...
    if use_w_in_combo and IsReady(game, w_spell) :
                target=TargetSelector(game,370)
                if target:
                    if  game.player.mana >= manaW[game.player.W.level -1]:
    ...
    if use_e_in_combo and IsReady(game, e_spell) :
                ...
                if target:
                    ...
                    if  game.player.mana >= manaE[game.player.E.level-1]:
                        if game.player.pos.distance(target.pos) >= playerAAdist: 
    ...
    if use_r_in_combo and IsReady(game, r_spell) :
                ...
                if targetR:
                            ...
                            if game.player.pos.distance (targetR.pos) <= playerAAdist :
...
def Laneclear(game):
    ...
    if lane_clear_with_q and IsReady(game, q_spell) :
        target = GetBestMinionsInRange (game,370)
        if target :
                if game.player.mana >= 40:
                        q_spell.move_and_trigger(game.world_to_screen(target.pos))
    ...
    if lane_clear_with_e and IsReady(game, e_spell) :
                ...
                target=GetBestMinionsInRange(game,650)
                if target:
...
def Jungleclear(game):
    ...
    if jungle_clear_with_q and IsReady(game, q_spell) :
        target = GetBestJungleInRange (game,370)
        if target :
                if game.player.mana >= 40:
                        q_spell.move_and_trigger(game.world_to_screen(target.pos))
    ...
    if jungle_clear_with_e and IsReady(game, e_spell) :
                ...
                target=GetBestJungleInRange(game,650)
                if target:
