...
def Combo(game):
    ...
    if use_w_in_combo and IsReady(game, w_spell) :
                ...
                if targetW :
                    if IsReady(game, e_spell) or IsReady(game, q_spell) :
                        w_spell.move_and_trigger(game.world_to_screen(targetW.pos))
                        
    if use_r_in_combo and IsReady(game, r_spell) :
                targetR=TargetSelector(game,700)
                if targetR:
                            e_travel_time = 700/20
                            predicted_pos = predict_pos (targetR, e_travel_time)
                            predicted_target = Fake_target (targetR.name, predicted_pos, targetR.gameplay_radius)
                            if game.player.pos.distance (predicted_target.pos) <= 700 and RDamage(game, targetR)>=targetR.health:
...
def Laneclear(game):
    ...
    if lane_clear_with_w and IsReady(game, w_spell) and IsReady(game, e_spell) or IsReady(game, q_spell) :
                targetW = GetBestMinionsInRange (game,650)
                if targetW :
                            w_spell.move_and_trigger(game.world_to_screen(targetW.pos))
...
def Jungleclear(game):
    ...
    if jungle_clear_with_w and IsReady(game, w_spell) and IsReady(game, e_spell) or IsReady(game, q_spell) :
                targetW = GetBestJungleInRange (game,650)
                if targetW :
                            w_spell.move_and_trigger(game.world_to_screen(targetW.pos))
