...
def AutoQ(game):
    ...
    if use_q_in_combo and IsReady(game, q_spell) :
                targetQ = GetBestTargetsInRange (game,550)
                if targetQ :
                            q_travel_time = 550/1200
                            ...
                            if game.player.pos.distance (predicted_target.pos) <= 540 :
                                if  game.player.mana >= mana_q[game.player.Q.level -1] and lastQ + 1 < game.time:
...
def Combo(game):
    ...
    if use_e_in_combo and IsReady(game, e_spell) :
                ...
                if targetQ :
                            # for buf in targetQ.buffs:
                            #     print(buf.name)
                            q_travel_time = 825/1500
                            predicted_pos = predict_pos (targetQ, q_travel_time)
                            predicted_target = Fake_target (targetQ.name, predicted_pos, targetQ.gameplay_radius)
                ...
                if targetQ2 :
                    hp = int(targetQ2.health / targetQ2.max_health * 100)

                    if getBuff(targetQ2,"AkaliEMis"):
                        if game.player.mana >= mana_q[game.player.Q.level -1] or hp<30:
