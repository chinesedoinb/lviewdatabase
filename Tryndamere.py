...
winstealer_script_info = {
    "script": "SA1-Tryndamere",
    "author": "SA1",
    "description": "SA1-Tryndamere",
    "target_champ": "tryndamere",
}
...
def winstealer_draw_settings(game, ui):
    ...
    ui.text("SA1-Tryndamere : 1.0.0.0")
    ui.separator ()
...
def Combo(game):
    ...
    if use_q_in_combo and IsReady(game, q_spell) :
                ...
                if hp<=30 and game.player.mana >=50:
                    if not getBuff(game.player, "UndyingRage"):
                        q_spell.trigger(False)

    if use_w_in_combo and IsReady(game, w_spell) :
                target=TargetSelector(game,850)
                if target:
                        w_spell.trigger(False)                                
    if use_e_in_combo and IsReady(game, e_spell) :
                targetQ = TargetSelector (game,650)
                if targetQ :
                            q_travel_time = 650/700
...
def Laneclear(game):
    ...
    if lane_clear_with_e and IsReady(game, e_spell) :
                ...
                if targetQ :
                            ...
                            predicted_pos = predict_pos (targetQ, q_travel_time)
                            predicted_target = Fake_target (targetQ.name, predicted_pos, targetQ.gameplay_radius)
                            if game.player.pos.distance (predicted_target.pos) <= 630:
